...
class Chromosome:
    def __init__(self, matrix, parents, parent1=None, parent2=None):
        self.matrix = matrix
        self.gens = [None for _ in range(self.matrix.count_of_vertex)]

        if parents:
            if parent2 is not None:
                self.gens = ordered_crossover(list1=parent1.gens, list2=parent2.gens)
            else:
    ...
    def mutation(self):
        ...
        for i in range(self.matrix.count_of_vertex):
            if random.randint(0, 1000) / 1000 < chance:
                ...
                while j == i:
                    j = random.randint(0, self.matrix.count_of_vertex - 1)
                self.gens = swap(list_2_swap=self.gens, i=i, j=j)
...
def crossover(chromosomes, chances, matrix):
    ...
    for _ in range((amount // 2) - 1):
        ...
        if random.randint(0, 100) < 85:
            new_generation.append(Chromosome(matrix=matrix, parents=True, parent1=parent1, parent2=parent2))
            new_generation.append(Chromosome(matrix=matrix, parents=True, parent1=parent2, parent2=parent1))
        else:
...
def genetic_algo(matrix, count_of_vertex):
    ...
    for _ in range(100):
        ...
        chromosomes.sort(key=lambda x: x.distance, reverse=True)
